...
- **"Remove bank" data-loss guard** (**DECIDED by Mason 2026-08-01 — option C, both halves**): `api/unlink-institution.js` (~46-58, SimpleFIN branch) cascade-deletes accounts+transactions; Restore (`api/simplefin-status.js:40-42`) only re-enables the org and a re-pull reaches ~88 days — so csv/pdf backfill rows on a fed account are lost in-app on a mis-tap. Build: (1) Remove becomes a soft-hide — accounts marked hidden/disabled, nothing deleted, Restore unhides them (matches the institution-tombstone philosophy); (2) a separate, deliberately buried "delete permanently" action for real cleanup (closed accounts), with a confirm that states what's lost. Every read path must respect the hidden state or dead accounts leak into totals.
