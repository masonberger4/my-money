...
- ~~Trends "biggest movers"~~ **SHIPPED 2026-08-03** — pure `biggestMovers` in `src/spending.js` (`isSpend()`/`spendingGroups` lineage, top 5 by |delta|, $1 noise floor) + `getBiggestMovers` on the range memo; its own purchase-based card on the Trends tab with month-tagged state, the two models never mixed in one number.
