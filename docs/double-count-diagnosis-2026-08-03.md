...
| F3 | Feed delivered duplicate rows with DISTINCT `sfin:` ids — upsert dedup cannot catch them. Payroll −$2,200 ×2 on 2026-07-24 (Cashback Debit); small Venture X same-day dupes (~$34 Jun+Jul) | July income ~$2,200 high | **RESOLVED — this row's `excluded=true` prescription was WRONG.** Verified against statements: payroll rows BOTH REAL (2026-08-09), Venture X same-day groups ALL REAL (2026-08-12). Do NOT exclude any copy; there were no feed dupes and July income was never overstated (CLAUDE.md Pending → Standing data rulings) |
| F4 | "Discover it (7933)" mistyped `depository/checking` under the Capital One org; twin exists as `credit` under the Discover org | $0 today (both hidden) | **RESOLVED 2026-08-10** — same card via two orgs (Capital One bought Discover): Capital One-org row unhidden + retyped Credit card; Discover-org row stays hidden permanently as the dedup guard (CLAUDE.md Pending) |
| F5 | NEWREZ mortgage in "Utilities" via keyword rule | ~$3.8k/mo mislabeled (counted once, correctly) | **RESOLVED by construction 2026-08-05** — the keyword table was deleted with the taxonomy; the rows read Uncategorized, just a merchant to teach |
| — | Checking (2644) history ends 2026-04-03 exactly where Checking (5481) starts — likely the same real account re-keyed by the feed | none (no overlap) | **HYPOTHESIS REFUTED 2026-08-11 (Mason)** — TWO REAL separate accounts (2644 personal/rarely used, 5481 the shared main checking); the abutment was coincidence. Each account's statements imported into its OWN row (backfill complete 2026-08-12) |
