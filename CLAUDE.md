...
- **Income** = `isIncome()` (`src/cashFlow.js`, the ONE income predicate —
  `isSpend`'s counterpart): unpaired depository inflows (checking or savings),
  money in from outside the boundary. Credit negatives are "Return"
  (`applyAccountRules`, unchanged): never income, never spending. `cashIncome`
...
- **The 4-type override (2026-08-15, Mason — YNAB vocabulary):**
  ...
  `user_type` > structure — the sign guards outrank the override (`'spending'`
  on money-in is never honored; honoring it would ADD a negative to
  `sumSpending`), loan rows ignore the override entirely (a later retype to
  loan must not resurrect it), and a non-null override beats BOTH card-payment
  vetoes. Income stays depository-only: `'inflow'` on a credit negative
  behaves like Return. Display: derived type restates these verdicts
...
vetted against main at synthesis time (re-verify the
