...
390×844). Recorded harness gap: the App.jsx institution-count Gotcha stays
untested — covering it needs a FIFTH full-match alias mocking
`supabaseClient.js`. Screenshot new UI before pushing. Tests (checked in, not gitignored):
...
- **Unmapped money is VISIBLE, never guessed** — same philosophy as
  ...
  taxonomy (2026-08-05); no category is pre-mapped to a line. The Schedule E
...
- **CI on every PR (2026-08-05, PRs #54/#55; hardened #59/#60)** —
  ...
  render gate over the checked-in `test/smoke/` harness that clicks ALL TEN
  tabs (the TDZ class ships green past tests AND build — only a real render
...
- **Self-hosting setup path (2026-08-08, PRs #64/#65, other session)** —
  `docs/SETUP.md` Path A (CLI `db push`, PENDING REHEARSAL) / Path B
...
- **`vercel.json` REJECTS unknown top-level keys** — schema validation fails
  ...
  Missing mechanism worth adding when touched next: a top-level-key allowlist
  assertion in `test/securityHeaders.test.js` (which already parses
  vercel.json), so an unknown key is a red test locally instead of a dead
  deploy pipeline.
