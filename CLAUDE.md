...
| `supabase/setup_all.sql` | One-paste fresh install — **DESTRUCTIVE, wipes all tables. Never run on live data. Never re-generate to include new migrations without that warning.** Convenience snapshot only — `migrations/` is the source of truth; ends with a column-level self-check that raises if it drifts behind migrations. **Known drift (2026-08-07): the snapshot AND its self-check stop at `20260731000001_receipts.sql`** — the five later migrations must be pasted after it on a fresh install; `docs/SETUP.md` lists them. |
