...
| `src/nav.js` | The bottom-nav map, pure + zero imports: `NAV_ITEMS` (5), `REFLECT_TABS`, `navForTab` (each of the 11 tab values → its nav item; unknown → home, never a highlight-less bar), `pageTitle`. The smoke harness walks these same ids via `data-mm-nav`/`data-mm-seg`/`data-mm-report`. `test/nav.test.js`. |
