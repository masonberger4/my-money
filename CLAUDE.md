...
| `src/reflect.js` | The Reflect hub's pure cores, zero imports: `breakdownSegments` (top-N + All Others off spendingGroups' output; segments CONSERVE the positive total by construction — pinned) and `incomeVsSpendingInsight` (avg income vs avg spending over getCashFlow's periods, ±10% band; returns null with no measured income — the hub then says "not enough" rather than claiming from $0; reads both the nested `{income:{amount}}` and plain shapes). This layer only ARRANGES numbers the shared model produced — it never re-derives spending or income. The hub lazy-loads cash flow via the trends effect (gate: `tab==="trends"||tab==="reflect"`). `test/reflect.test.js`. |
