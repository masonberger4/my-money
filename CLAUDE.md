...
| `src/db.js` | getSetting/setSetting (+ `getSettings` batch read, `deleteSetting`) on the Supabase `settings` table (dashboard prefs: colors, names, custom categories, `asst:model`/`asst:effort`). Since Session D, ALL settings-table I/O routes through here — no direct `.from('settings')` elsewhere. |
