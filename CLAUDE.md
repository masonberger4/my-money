...
| `api/_lib/unlink.js` | Pure remove-bank decisions, zero I/O: the `unlink:<institutionId>` settings-key namespacing, `visibleAccountIds` (which account ids to record), tolerant `parseRestoreSet`, `restoreSet` (recorded ∩ still-present — deliberately-hidden and post-remove-arrival accounts never unhidden), and the `permanent:true`+`confirm:'delete'` literal gate. `test/unlink.test.js`. |
...
- ~~Verify the $2,200 payroll duplicate~~ **RESOLVED 2026-08-09 — REAL, keep
  both rows.** The Discover July 2026 statement itself lists "ACH Deposit
  PAYROLL From POME HOLISTIC PE" $2,200 TWICE on Jul 24, and both are inside
