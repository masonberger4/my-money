...
- **Income** = `isIncome()` (`src/cashFlow.js`, the ONE income predicate —
  ...
  money in from outside the boundary. Credit negatives are refunds or payments
  received: never income, and since 2026-08-17 a refund is negative SPENDING
  while a payment received is vetoed out of both. `cashIncome`
...
- (2026-08-17) **The card-payment veto is CALIBRATED against the household's
  ...
  10/10 payment/reward wordings are held out of spending ($31,202.99, of which
  $26,238 is `CAPITAL ONE MOBILE PYMT`) and 21/21 merchant refunds net
  ($3,692.71). Both lists are pinned BY NAME in `test/txClassify.test.js`. The
...
- (2026-08-17) **DEBIT-card refunds are effectively absent from this
  ...
  depository money-in, the only refund-shaped rows are a $24.85 Comcast
  credit, a $3.00 out-of-network ATM fee refund and $0.30 of Schwab
  account-verification micro-deposits — about $28 total. Everything else is
  payroll (`MOONLIGHT BIO`, `SYSTIMMUNE`, `POME HOLISTIC`), `WA ST EMPLOY SEC`
...
- (2026-08-17) **OPEN, not a bug — recurring `Zelle Payment From <person>`
  ...
  (DILPREET KOCHHAR ×7, SUJANI DWARAKANATH ×5, ASTHA SHARMA ×3). If they are
