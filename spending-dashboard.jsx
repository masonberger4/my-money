...
const ERA_MCP = "https://context.era.app";
...
function parseToolResult(content) {
  ...
  for (const b of content.filter(b => b.type === "mcp_tool_result")) {
...
async function callEra(toolName, toolInput = {}) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      model: MODEL, max_tokens: 1000,
      system: "Call the requested tool immediately and return the raw result only.",
      messages: [{ role: "user", content: `Call ${toolName} with: ${JSON.stringify(toolInput)}` }],
      mcp_servers: [{ type: "url", url: ERA_MCP, name: "era" }],
    }),
  });
  return parseToolResult((await res.json()).content || []);
}
...
export default function Dashboard() {
    ...
    async function load(){
      try {
        const [c,n,cc]=await Promise.all([
          window.storage.get("dash:colors").catch(()=>null),
          window.storage.get("dash:names").catch(()=>null),
          window.storage.get("dash:cats").catch(()=>null),
        ]);
        if(c)setCustomColors(JSON.parse(c.value));
        if(n)setCustomNames(JSON.parse(n.value));
        if(cc)setCustomCats(JSON.parse(cc.value));
      } catch{}
      setReady(true);
    }
    load();
  ...
  async function saveColors(next){setCustomColors(next);try{await window.storage.set("dash:colors",JSON.stringify(next));}catch{}}
  async function saveNames(next){setCustomNames(next);try{await window.storage.set("dash:names",JSON.stringify(next));}catch{}}
  async function saveCats(next){setCustomCats(next);try{await window.storage.set("dash:cats",JSON.stringify(next));}catch{}}
  ...
  const fetchData=useCallback(async(y,m)=>{
    ...
    try{
      const[ov,sp,tx,cf]=await Promise.all([
        cur?callEra("knowledge__get_financial_context_and_overview",{}):Promise.resolve(null),
        callEra("insights__analyze_spending",{group_by:"category",period:"custom",period_start:s,period_end:e,include_subcategories:false,top_n:15}),
        callEra("transactions__list_transactions",{from_date:s,to_date:e,page_size:50,page:1}),
        callEra("insights__get_cash_flow",{granularity:"month",num_periods:6}),
      ]);
      setOverview(ov);setSpending(sp);setTransactions(tx);setCashFlow(cf);
      setLastUpd(new Date());
    }catch{setError("Couldn't load data. Check your Era connection.");}
    setLoading(false);
  },[]);

  useEffect(()=>{if(ready)fetchData(year,month);},[year,month,ready]);
    ...
    <div style={{fontFamily:"'DM Sans','Helvetica Neue',sans-serif",background:"var(--bg,#F7F6F2)",minHeight:"100vh",
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"24px 16px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <button className="ibtn" onClick={()=>fetchData(year,month)} disabled={loading}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
          {[
            ...
            {label:"Card balance",val:loading?null:fmtX(balance),sub:"Capital One Venture X"},
        ...
        <div style={{textAlign:"center",marginTop:18,fontSize:11,color:"var(--muted)"}}>Capital One Venture X · Era Context</div>
      </div>
