...
export function formatSpendingContext(accounts, txs, extras = {}) {
  ...
  lines.push('Transaction amounts (unlike the balances above): positive is money out. These totals already apply the app\'s spending rule, the same one the dashboard uses: a REFUND on a credit card is negative and SUBTRACTS from its category (so a returned item nets to zero); money into a checking or savings account is income and does not count as spending unless the household explicitly marked that row a Refund, in which case it subtracts like a card refund; money OUT of an account that they marked Income is income given back (an overpayment returned to its sender) and LOWERS the income for that month rather than counting as spending; a row the household marked Income counts as income whatever account it sits on, including a credit card, though only checking and savings are counted as income automatically; card payments never count; and a transfer counts UNLESS it was matched to an equal-amount opposite leg on another of the household\'s own visible accounts. A category total can therefore be negative when refunds outweighed purchases. Quote these totals rather than re-adding the transaction rows below.');
  if (partialMonth) {
    lines.push(
